"""Testes para o serializador 'ProductDetailSerializer'."""

import pytest
from rest_framework import serializers
from app.serializers import ProductDetailSerializer # pylint: disable=import-error
from faker import Faker


def test_quantity_greater_than_zero(faker: Faker):
    """
    Valida o método 'validate_quantidade' do 'ProductDetailSerializer'.
    Deve retornar 'True' quando a quantidade é maior que zero.
    """

    for n in [faker.random_int(min=1, max=10**5) for _ in range(10)]:
        assert ProductDetailSerializer().validate_quantidade(n)
...
def test_quantity_equal_to_zero():
    ...
    assert ProductDetailSerializer().validate_quantidade(0) == 0
...
def test_quantity_less_than_zero(faker: Faker):
    ...
    for n in [faker.random_int(min=-10**5, max=-1) for _ in range(10)]:
        with pytest.raises(serializers.ValidationError):
            ProductDetailSerializer().validate_quantidade(n)


def test_price_greater_than_zero(faker: Faker):
    """
    Valida o método 'validate_preco' do 'ProductDetailSerializer'.
    ...
    for n in [faker.pyfloat(left_digits=3, right_digits=2, positive=True) for _ in range(10)]:
        assert ProductDetailSerializer().validate_preco(n)
...
def test_price_equal_to_zero():
    ...
    with pytest.raises(serializers.ValidationError):
        ProductDetailSerializer().validate_preco(0)
...
def test_price_less_than_zero(faker: Faker):
    ...
    for n in [faker.pyfloat(left_digits=3, right_digits=2, positive=False) for _ in range(10)]:
        with pytest.raises(serializers.ValidationError):
            ProductDetailSerializer().validate_preco(n)


def test_height_greater_than_zero(faker: Faker):
    """
    Valida o método 'validate_altura' do 'ProductDetailSerializer'.
    Deve retornar 'True' quando a altura é maior que zero.
    """

    for n in [faker.pyfloat(left_digits=3, right_digits=2, positive=True) for _ in range(10)]:
        assert ProductDetailSerializer().validate_altura(n)
...
def test_height_equal_to_zero():
    ...
    with pytest.raises(serializers.ValidationError):
        ProductDetailSerializer().validate_altura(0)
...
def test_width_greater_than_zero(faker: Faker):
    """
    Valida o método 'validate_largura' do 'ProductDetailSerializer'.
    Deve retornar 'True' quando a largura é maior que zero.
    """

    for n in [faker.pyfloat(left_digits=3, right_digits=2, positive=True) for _ in range(10)]:
        assert ProductDetailSerializer().validate_largura(n)
...
def test_width_equal_to_zero():
    ...
    with pytest.raises(serializers.ValidationError):
        ProductDetailSerializer().validate_largura(0)
...
def test_length_greater_than_zero(faker: Faker):
    """
    Valida o método 'validate_comprimento' do 'ProductDetailSerializer'.
    Deve retornar 'True' quando a comprimento é maior que zero.
    """

    for n in [faker.pyfloat(left_digits=3, right_digits=2, positive=True) for _ in range(10)]:
        assert ProductDetailSerializer().validate_comprimento(n)
...
def test_length_equal_to_zero():
    ...
    with pytest.raises(serializers.ValidationError):
        ProductDetailSerializer().validate_comprimento(0)
